...
[RequireChatAdmin]
[Command("/rmallchat")]
[Description("Удалить все напоминания в текущем чате (для админов чата)")]
internal sealed class DeleteAllChatHandler : ICommandHandler
{
    private readonly ITelegramBotClient _bot;
    private readonly AppDbContext _db;
    private readonly ISchedulingService _sched;

    public DeleteAllChatHandler(ITelegramBotClient bot, AppDbContext db, ISchedulingService sched)
    {
        _bot = bot;
        _db = db;
        _sched = sched;
    ...
    public async Task Execute(CommandContext ctx)
    {
        var list = await _db.Reminders
            .Where(x => x.ChatId == ctx.ChatId)
            .Select(x => x.Id)
            .ToListAsync(ctx.CancellationToken);

        if (list.Count == 0)
        {
            await _bot.SendMessage(ctx.Message.Chat, "В этом чате напоминаний нет.",
        ...
        await _bot.SendMessage(ctx.Message.Chat, $"Удалено всех напоминаний в чате: {list.Count}",
            replyParameters: ReplyHelper.R(ctx.Message), messageThreadId: ctx.ThreadId, cancellationToken: ctx.CancellationToken);
    }
}
